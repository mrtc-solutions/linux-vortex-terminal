...
async function loadEngagements() { try { const data = await api('/api/engagements'); state.engagements = data.engagements; const current = state.engagements.find(e => e.id === state.activeEngagementId); if (!current || current.status !== 'active') state.activeEngagementId = (state.engagements.find(e => e.status === 'active') || {}).id || null; renderEngagements(); } catch(e) { toast(e.message, true); } }
