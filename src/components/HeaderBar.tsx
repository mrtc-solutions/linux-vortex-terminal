      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="hidden lg:flex items-center text-xs text-stone-400 font-mono border-l border-[var(--theme-border)] pl-3 space-x-2">
            <span className="text-[var(--theme-dim)]">ghost@spectre-box:</span>
            <span className="text-xs text-[var(--theme-primary)]">POSIX x86_64</span>
            <span className="text-xs bg-emerald-950/80 text-emerald-400 px-1 rounded border border-emerald-800/50">
              Agent-Reach: ARMED
            </span>
          </div>

          {/* Local LLM Cluster Health Badges */}
          <div className="hidden xl:flex items-center space-x-1.5 text-[11px] font-mono border-l border-[var(--theme-border)] pl-3">
            {LOCAL_MODELS.map((m) => (
              <div
                ...
                title={`${m.name} (${m.quantization}) | VRAM: ${m.vramUsageMB}MB | Latency: ${m.latencyMs}ms`}
              >
                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: m.color }} />
                <span className="font-medium">{m.name.split('-')[0]}</span>
                <span className="text-stone-500 text-[10px]">({m.parameters})</span>
              </div>
            ))}
          </div>
        </div>
