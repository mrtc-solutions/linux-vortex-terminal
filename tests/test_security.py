...
class VortexSecurityTests(unittest.TestCase):
    ...
    def test_guardian_does_not_false_positive_adduser(self):
        ...
        self.assertTrue(looks_destructive("unlink /tmp/foo"))
        self.assertTrue(looks_destructive("truncate -s 0 /tmp/foo"))
        self.assertTrue(looks_destructive("/sbin/mkswap /dev/sda2"))
        self.assertTrue(looks_destructive("find /tmp -name '*.log' -delete"))
        self.assertTrue(looks_destructive("/usr/bin/find /tmp -delete"))
        self.assertFalse(looks_destructive("find /tmp -name '*.log'"))
        self.assertFalse(looks_destructive("cat /tmp/unlink.log"))
        self.assertFalse(looks_destructive("nano /tmp/truncate.txt"))
