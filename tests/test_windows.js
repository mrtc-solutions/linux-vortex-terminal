...
assert.ok(html.includes('Minimise') && html.includes('Maximise') && html.includes('Exit'), 'native window chrome labels minimise, maximise, exit');
assert.ok(html.includes('Host dependencies · present + missing'), 'legacy dependencies window shows present and missing');
...
assert.ok(fs.existsSync(iconPng), 'npm start Electron window needs a real PNG icon on disk');
assert.deepStrictEqual([...fs.readFileSync(iconPng).subarray(0, 8)], [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
assert.ok(main.includes("assets', 'icons', 'vortex.png'"), 'Electron BrowserWindow must load the real Vortex icon');
assert.ok(main.includes('nativeImage'), 'Electron icon is a real nativeImage, not a placeholder');
...
assert.strictEqual(isAllowedApiRequest('/api/reports/abc123/delete', 'POST'), true);
assert.strictEqual(isAllowedApiRequest('/api/reports/abc123/rename', 'POST'), true);
assert.strictEqual(isAllowedApiRequest('/api/reports/abc123/edit', 'POST'), true);
assert.strictEqual(isAllowedApiRequest('/api/reports/abc123/rename', 'GET'), false);
